...
2. **Add the label `Cursor-sourced`** to the issue (via Linear MCP update_issue) so it's counted as pipeline-sourced for metrics. **Check for the label at the team level:** call `list_issue_labels` with the issue's team (e.g. `team: "SYT Labs"` or the team name/ID from the issue); in Linear, `Cursor-sourced` is often a **team** label, not a workspace label. If the label exists for that team, add it; if not, skip and mention that the user can create it in Linear (Settings → Labels, including team labels).
...
If the issue is **already In Progress or Done**, still add a **comment** with the spec path and origin; add the label **`Cursor-sourced`** if it exists for the issue's team (see above). Only change assignee/state if the user asks.
...
- **Create** a Linear issue with:
  ...
  - **Labels:** Include **`Cursor-sourced`** so the issue is tagged as coming from the Signal-to-Code pipeline (enables metrics: "issues sourced by Cursor"). **Check team labels:** call `list_issue_labels` with the target team (e.g. the team you're creating the issue in); if `Cursor-sourced` exists for that team, add it. Add any other labels the user asked for.
...
**`Cursor-sourced` and `Cursor-built` are often team labels.** When checking whether they exist, call `list_issue_labels` with the **team** parameter (e.g. the issue's team or the team you're creating the issue in). Listing without `team` returns only workspace-level labels and can miss them. If the team has these labels (create in Linear: Settings → Labels, including team labels), the pipeline can tag issues for reporting: "Cursor-sourced" = issue created or enriched from our spec/signal pipeline; "Cursor-built" = implementation done via `/do-linear-ticket` or `/signal-to-pr`. You can then filter or report on those labels in Linear.
